...
function get_download_meta($opt)
{
    ...
    if (isset($options[$opt]['url'])) {
        return $options[$opt]['url'];
...
add_action( 'elementor/element/after_section_end', function( $element, $section_id, $args ) {
    ...
    if ( 'section' === $element->get_name() && 'section_custom_css_pro' === $section_id ) {
        ...
        $element->add_control(
            'header_sticky',
            [
                'label'        =>   __( 'Sticky Section', 'maan' ),
                'type'         => \Elementor\Controls_Manager::SWITCHER,
                'label_on'     =>   __( 'Yes', 'maan' ),
                'label_off'    =>   __( 'No', 'maan' ),
                'return_value' => 'maan-sticky-header',
                'default'      => '',
                'prefix_class' => '',
            ]
        );
        $element->add_group_control(
            ...
            [
                'name' => 'sticky_bg',
                'label' => __( 'Main BG', 'maan' ),
                'types' => [ 'classic', 'gradient' ],
                'show_label' => true,
                'condition' => [
                    'header_sticky' => 'maan-sticky-header'
                ],
                'selector' => '{{WRAPPER}}.maan-sticky-header.sticky-on',
            ]
        );
        $element->add_responsive_control(
            'sticky_padding',
            [
                'label' =>   esc_html__('Padding', 'maan'),
                ...
                'selectors' => [
                    '{{WRAPPER}}.maan-sticky-header.sticky-on' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ]
            ]
        );
        ...
        $element->add_responsive_control(
            ...
            [
                ...
                'options' => [
                    '' => __( 'Default', 'maan' ),
                    'inherit' => __( 'Full Width', 'maan' ) . ' (100%)',
                    'auto' => __( 'Inline', 'maan' ) . ' (auto)',
                    'initial' => __( 'Custom', 'maan' ),
                ],
        ...
        $element->add_control(
            ...
            [
                'raw' => '<strong>' . __( 'Please note!', 'maan' ) . '</strong> ' . __( 'Custom positioning is not considered best practice for responsive web design and should not be used too frequently.', 'maan' ),
        ...
        $element->add_control(
            '_position',
            [
                'label' => __( 'Position', 'maan' ),
                'type' => \Elementor\Controls_Manager::SELECT,
                'default' => '',
                'options' => [
                    '' => __( 'Default', 'maan' ),
                    'absolute' => __( 'Absolute', 'maan' ),
                    'fixed' => __( 'Fixed', 'maan' ),
                ],
        ...
        $start = is_rtl() ? __( 'Right', 'maan' ) : __( 'Left', 'maan' );
        $end = ! is_rtl() ? __( 'Right', 'maan' ) : __( 'Left', 'maan' );

        $element->add_control(
            '_offset_orientation_h',
            [
                'label' => __( 'Horizontal Orientation', 'maan' ),
                'type' => \Elementor\Controls_Manager::CHOOSE,
                'toggle' => false,
                'default' => 'start',
                'options' => [
        ...
        $element->add_control(
            ...
            [
                'label' => __( 'Vertical Orientation', 'maan' ),
                'type' => \Elementor\Controls_Manager::CHOOSE,
                'toggle' => false,
                'default' => 'start',
                'options' => [
...
add_action( 'elementor/element/heading/section_title_style/before_section_start', function( $element, $args ) {
    ...
    $element->add_control(
        'text_transparent',
        [
            'label'        =>   __( 'Transparent', 'maan' ),
            'type'         => \Elementor\Controls_Manager::SWITCHER,
            'label_on'     =>   __( 'Yes', 'maan' ),
            'label_off'    =>   __( 'No', 'maan' ),
            'return_value' => 'maan-text-gradient',
            'default'      => 'false',
            'prefix_class' => '',
        ]
    );
    $element->add_group_control(
        ...
        [
            ...
            'separator' => 'after',
            'condition' => [
                'text_transparent' => 'maan-text-gradient'
            ],
            'selector' => '{{WRAPPER}}.maan-text-gradient .elementor-heading-title',
        ]
    );
    $element->add_control(
        'text_sr',
        [
            'label' => __( 'Text Stroke', 'maan' ),
            'type' => \Elementor\Controls_Manager::HEADING,
        ]
    );
    $element->add_control(
        'text_str',
        [
            'label'        =>   __( 'Turn On Stroke', 'maan' ),
            'type'         => \Elementor\Controls_Manager::SWITCHER,
            'label_on'     =>   __( 'Yes', 'maan' ),
            'label_off'    =>   __( 'No', 'maan' ),
            'return_value' => 'maan-text-stroke',
            'default'      => 'false',
            'prefix_class' => '',
        ]
    );
    $element->add_control(
        's_fill_color',
        [
            'label' => __( 'Fill Color', 'maan' ),
            'type' => \Elementor\Controls_Manager::COLOR,
            'condition' => [
                'text_str' => 'maan-text-stroke'
            ],
            'selectors' => [
                '{{WRAPPER}}.maan-text-stroke .elementor-heading-title' => '-webkit-text-fill-color: {{VALUE}}',
            ],
        ]
    );
    $element->add_responsive_control(
        ...
        [
            ...
            'selectors' => [
                '{{WRAPPER}}.maan-text-stroke .elementor-heading-title' => '-webkit-text-stroke-width: {{SIZE}}{{UNIT}};',
            ],
            'condition' => [
                'text_str' => 'maan-text-stroke'
            ],
    ...
    $element->add_control(
        's_text_color',
        [
            'label' => __( 'Stroke Color', 'maan' ),
            'type' => \Elementor\Controls_Manager::COLOR,
            'condition' => [
                'text_str' => 'maan-text-stroke'
            ],
            'selectors' => [
                '{{WRAPPER}}.maan-text-stroke .elementor-heading-title' => '-webkit-text-stroke-color: {{VALUE}}',
            ],
        ]
    );
    $element->end_controls_section();
...
add_action( 'elementor/element/button/section_style/before_section_start', function( $element, $args ) {
    ...
    $element->start_controls_section(
        ...
        [
            'tab' => \Elementor\Controls_Manager::TAB_STYLE,
            'label' => __( 'Maan Settings', 'appilo' ),
        ]
    );
    $element->add_control(
        'button_before',
        [
            'label'        =>   __( 'Turn On Before', 'maan' ),
            'type'         => \Elementor\Controls_Manager::SWITCHER,
            'label_on'     =>   __( 'Yes', 'maan' ),
            'label_off'    =>   __( 'No', 'maan' ),
            'return_value' => 'maan-before',
            'default'      => 'maan-before-none',
            'prefix_class' => '',
        ]
    );
    $element->add_group_control(
        ...
        [
            'name' => 'before_bg',
            'label' => __( 'Button Background', 'maan' ),
            'types' => [  'classic', 'gradient' ],
            'show_label' => true,
            'separator' => 'after',
            'condition' => [
                'button_before' => ['maan-before']
            ],
            'selector' => '{{WRAPPER}}.maan-before .elementor-button:before',
        ]
    );
    $element->add_responsive_control(
        'bra_btn',
        [
            'label' => __( 'Border Radius', 'maan' ),
            'type' => \Elementor\Controls_Manager::DIMENSIONS,
            'size_units' => [ 'px', '%', 'em' ],
            'condition' => [
                'button_before' => ['maan-before']
            ...
            'selectors' => [
                '{{WRAPPER}}.maan-before .elementor-button:before' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
            ],
        ]
    );
...
add_action( 'elementor/element/icon/section_style_icon/before_section_start', function( $element, $args ) {
    ...
    $element->add_control(
        'icon_light_box',
        [
            'label'        =>   __( 'Turn On LightBox', 'maan' ),
            'type'         => \Elementor\Controls_Manager::SWITCHER,
            'label_on'     =>   __( 'Yes', 'maan' ),
            'label_off'    =>   __( 'No', 'maan' ),
            'return_value' => 'maan-icon-lightbox',
            'default'      => 'false',
            'prefix_class' => '',
        ]
    );
    $element->add_control(
        'icon_hotspot',
        [
            'label'        =>   __( 'Turn On Hotspot', 'maan' ),
            'type'         => \Elementor\Controls_Manager::SWITCHER,
            'label_on'     =>   __( 'Yes', 'maan' ),
            'label_off'    =>   __( 'No', 'maan' ),
            'return_value' => 'maan-icon-hotspot',
            'default'      => 'false',
            'prefix_class' => '',
        ]
    );
    $element->add_control(
        'icon_gradient',
        [
            'label'        =>   __( 'Turn On Gradient', 'maan' ),
            'type'         => \Elementor\Controls_Manager::SWITCHER,
            'label_on'     =>   __( 'Yes', 'maan' ),
            'label_off'    =>   __( 'No', 'maan' ),
            'return_value' => 'maan-icon-gradient',
            'default'      => 'false',
            'prefix_class' => '',
        ]
    );
    ...
    $element->start_controls_tab(
        ...
        [
            'label' => __( 'Normal', 'appilo' ),
            'condition' => [
                'icon_gradient' => 'maan-icon-gradient'
            ],
        ]
    );
    $element->add_control(
    ...
    $element->add_group_control(
        ...
        [
            ...
            'show_label' => true,
            'separator' => 'after',
            'selector' => '{{WRAPPER}}.maan-icon-gradient .elementor-icon i',
        ]
    );
    $element->add_control(
        'atext_gra',
    ...
    $element->add_group_control(
        ...
        [
            ...
            'show_label' => true,
            'separator' => 'after',
            'selector' => '{{WRAPPER}}.maan-icon-gradient .elementor-icon',
        ]
    );
    $element->end_controls_tab();
    ...
    $element->add_group_control(
        ...
        [
            ...
            'show_label' => true,
            'separator' => 'after',
            'selector' => '{{WRAPPER}}.maan-icon-gradient .elementor-icon:hover i',
        ]
    );
    $element->add_control(
        'afatext_gra',
    ...
    $element->add_group_control(
        ...
        [
            ...
            'separator' => 'after',
            'selector' => '{{WRAPPER}}.maan-icon-gradient .elementor-icon:hover',
        ]
    );
    $element->end_controls_tab();
...
function maan_list_control($settings, $icon, $tag="li"){
    if(!empty($settings)){
...
function maanelement_template_select() {
...
/*Meta Fields*/
function maanelement_animations(){
    return array(
        '' => esc_html__('No animation', 'maanelement') ,
        'reveal-top' => esc_html__('Animate from top', 'maanelement') ,
        'reveal-bottom' => esc_html__('Animate from bottom', 'maanelement') ,
        'reveal-left' => esc_html__('Animate from left', 'maanelement') ,
        'reveal-right' => esc_html__('Animate from right', 'maanelement') ,
    );  
}
...
function maanelement_gird_column($column_size = '3') {
...
function maanelement_gallery_column($column_size = 3) {
...
function maan_page_title($arg){
        ...
        if ( is_category() ) {
          ...
          $title = esc_html__( 'Frontpage', 'maan' );
        } elseif( is_singular() ){
          $title = get_the_title();
        }else {
            $title = esc_html__( 'Archives','maan' );
        }
